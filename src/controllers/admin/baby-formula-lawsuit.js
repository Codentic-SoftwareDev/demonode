...
const list = async (req, res) => {
  ...
  await babyFormulaLawsuitModel
    ...
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await babyFormulaLawsuitModel.count()
...
// remove/delete baby formula lawsuit
  ...
  await babyFormulaLawsuitModel
    .find(
      {},
      {
        ...
        message: 1,
        chid_dob: 1,
        was_the_child_was_born_premature: 1,
        was_the_child_given_formula_or_milk_fortifier: 1,
        was_the_child_diagnosed_with_nec: 1,
        did_the_child_experience_any_of_the_following: 1,
        agree_t_and_c: 1,
        ip_address: 1,
        created_at: 1
      }
    ...
    .exec((err, data) => {
      ...
      const fileName = 'baby-formula-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/baby-formula-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/baby-formula-lawsuit-excel-file/' + fileName
