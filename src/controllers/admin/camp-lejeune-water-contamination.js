...
// List camp lejeune water contamination
const list = async (req, res) => {
  const i18n = req.i18n
  const page = parseInt(req.query.page)
  const itemsPerPage = parseInt(req.query.items_per_page)
  const offset = (page - 1) * itemsPerPage
  await campLejeuneWaterContaminationModel
    .find()
    .skip(offset)
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await campLejeuneWaterContaminationModel.count()
...
// View camp lejeune water contamination
...
// remove/delete camp lejeune water contamination
...
const exportData = async (req, res) => {
  ...
  await campLejeuneWaterContaminationModel
    ...
    .exec((err, data) => {
      ...
      const fileName = 'camp-lejeune-water-contamination-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/camp-lejeune-water-contamination'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/camp-lejeune-water-contamination/' + fileName
...
module.exports.campLejeuneWaterContaminationController = {
  list,
  view,
  remove,
  exportData
