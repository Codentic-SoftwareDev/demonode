...
const list = async (req, res) => {
  ...
  await elmironLawsuitModel
    ...
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await elmironLawsuitModel.count()
...
const exportData = async (req, res) => {
  ...
  await elmironLawsuitModel
    ...
    .exec((err, data) => {
      ...
      const fileName = 'elmiron-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/elmiron-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/elmiron-lawsuit-excel-file/' + fileName
