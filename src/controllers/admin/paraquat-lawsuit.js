...
// List paraquat lawsuit
const list = async (req, res) => {
  const i18n = req.i18n
  const page = parseInt(req.query.page)
  const itemsPerPage = parseInt(req.query.items_per_page)
  const offset = (page - 1) * itemsPerPage
  await paraquatLawsuitModel
    .find()
    .skip(offset)
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await paraquatLawsuitModel.count()
...
// remove/delete paraquat lawsuit
  ...
  await paraquatLawsuitModel
    .find(
      {},
      {
        ...
        zip: 1,
        message: 1,
        are_you_liscensed_to_use_Paraquat: 1,
        were_you_given_medication: 1,
        are_you_working_around_commercial_weed_killers_and_pesticides: 1,
        diagnosed_with_Parkinson_disease_with_movement_disorder: 1,
        agree_t_and_c: 1,
        ip_address: 1,
    ...
    .exec((err, data) => {
      ...
      const fileName = 'paraquat-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/paraquat-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/paraquat-lawsuit-excel-file/' + fileName
...
module.exports.paraquatLawsuitController = {
  list,
  view,
  remove,
  exportData
