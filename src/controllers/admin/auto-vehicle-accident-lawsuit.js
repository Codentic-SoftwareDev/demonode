...
// List auto vehicle accident lawsuit
const list = async (req, res) => {
  ...
  await autoVehicleAccidentLawsuitModel
    ...
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await autoVehicleAccidentLawsuitModel.count()
...
// View auto vehicle accident lawsuit
...
// remove/delete auto vehicle accident lawsuit
  ...
  await autoVehicleAccidentLawsuitModel
    .find(
      {},
      {
        ...
        zip: 1,
        message: 1,
        was_the_accident_in_the_last_1_years: 1,
        did_you_report_the_accident_to_the_police: 1,
        did_you_seek_medical_attention: 1,
        do_you_have_ongoing_medical_needs: 1,
        agree_t_and_c: 1,
        ip_address: 1,
    ...
    .exec((err, data) => {
      ...
      const fileName = 'auto-vehicle-accident-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/auto-vehicle-accident-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/auto-vehicle-accident-lawsuit-excel-file/' + fileName
