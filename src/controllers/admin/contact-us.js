...
const list = async (req, res) => {
  ...
  await contactUsModel
    ...
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await contactUsModel.count()
...
const exportData = async (req, res) => {
  ...
  await contactUsModel
    ...
    .exec((err, data) => {
      ...
      const fileName = 'contact-us-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/contact-us-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/contact-us-excel-file/' + fileName
