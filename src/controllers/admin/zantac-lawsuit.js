...
const list = async (req, res) => {
  ...
  await zanTacModel
    ...
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await zanTacModel.count()
  ...
  await zanTacModel
    .find(
      {},
      {
        ...
        zip: 1,
        message: 1,
        you_or_your_loved_one_used_Zantac_before: 1,
        you_or_a_loved_one_diagnosed_with_Cancer: 1,
        agree_t_and_c: 1,
        ip_address: 1,
    ...
    .exec((err, data) => {
      ...
      const fileName = 'zantac-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/zantac-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/zantac-lawsuit-excel-file/' + fileName
...
module.exports.zanTacController = {
  list,
  view,
  remove,
  exportData
