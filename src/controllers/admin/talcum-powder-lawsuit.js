...
const list = async (req, res) => {
  ...
  await talcumPowderLawsuitModel
    .find()
    .skip(offset)
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await talcumPowderLawsuitModel.count()
  ...
  await talcumPowderLawsuitModel
    .find(
      {},
      {
        ...
        zip: 1,
        message: 1,
        talc_and_had_a_diagnosis_of_ovarian_cancer_anytime_from_to_current: 1,
        agree_t_and_c: 1,
        ip_address: 1,
    ...
    .exec((err, data) => {
      ...
      const fileName = 'talcum-powder-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/talcum-powder-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/talcum-powder-lawsuit-excel-file/' + fileName
...
module.exports.talcumPowderLawsuitController = {
  list,
  view,
  remove,
  exportData
