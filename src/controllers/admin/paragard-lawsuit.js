...
const paragardLawsuit = require('../../models/paragard-lawsuit')
...
const list = async (req, res) => {
  ...
  await paragardLawsuit
    .find()
    .skip(offset)
    .limit(itemsPerPage)
    .then(async result => {
      const totalRecords = await paragardLawsuit.count()
  ...
  await paragardLawsuit
    .find(
      {},
      {
        ...
        zip: 1,
        message: 1,
        are_you_concerned_about_an_injury_from_a_paragard_IUD_device_implant: 1,
        is_the_injured_individual_you_or_a_loved_one: 1,
        what_date_was_the_Paragard_IUD_implanted: {
          $dateToString: { format: '%d-%m-%Y', date: '$what_date_was_the_Paragard_IUD_implanted' }
        },
        was_there_any_other_complications_when_the_IUD_broke: 1,
        was_surgery_required_to_remove_the_paragard_IUD: 1,
        have_you_or_your_loved_one_ever_signed_any_documents_retainers_or_agreements_with_an_attorney_or_a_law_firm_about_this_before: 1,
        agree_t_and_c: 1,
        ip_address: 1,
    ...
    .exec((err, data) => {
      ...
      const fileName = 'paragard-lawsuit-' + dummyName + '.csv'
      const excelFileDir = 'public/upload/paragard-lawsuit-excel-file'
      if (!fs.existsSync(excelFileDir)) {
        fs.mkdirSync(excelFileDir, { recursive: true })
      }
      const filePath = 'public/upload/paragard-lawsuit-excel-file/' + fileName
